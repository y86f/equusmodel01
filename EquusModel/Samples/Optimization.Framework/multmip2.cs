...
namespace EquusModel.Samples.Optimization.Framework
{
    class multmip2
    {
        ...
        static Model BuildModel()
        {
            ...
            multimip2Model.Destinations = destinations;
            multimip2Model.Origins = origins;
            multimip2Model.Products = products;
            ...
            mathModel.AddObjective(
                Expression.Sum(multimip2Model.Origins.SelectMany(orig => orig.Vcost.Select(vcostlist => vcostlist.Item3 * Transport[orig, vcostlist.Item1, vcostlist.Item2]))) + Expression.Sum(multimip2Model.Origins.SelectMany(orig => multimip2Model.Destinations.Select(dest => orig.Fcost[dest] * Use[orig, dest]))),
                "z"
                );

            // Supply
            foreach (Origin orig in multimip2Model.Origins)
            {
                foreach (Product prod in multimip2Model.Products)
                {
                    var expression = Expression.Sum(multimip2Model.Destinations.Select(dest => Transport[orig, dest, prod]));
                    mathModel.AddConstraint(expression == orig.Supply[prod]);
                }
            }

            // Demand
            foreach (Destination dest in multimip2Model.Destinations)
            {
                foreach (Product prod in multimip2Model.Products)
                {
                    var expression = Expression.Sum(multimip2Model.Origins.Select(orig => Transport[orig, dest, prod]));
            ...
            foreach (Origin orig in multimip2Model.Origins)
            {
                foreach (Destination dest in multimip2Model.Destinations)
                {
                    var expression = Expression.Sum(multimip2Model.Products.Select(prod => Transport[orig, dest, prod]));
